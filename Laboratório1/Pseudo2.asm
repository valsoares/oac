...
.text
li a2,N
la t2,coordenadas #peguei o endere√ßo do vetor onde vou guardar as words coordenadas
sw a2,0(t2) #primeiro elemento √© o n√∫mero de elementos no vetor coordenadas
...
or a0,a0,t1 #opera√ß√£o or(x+y) em a0. Ex: 0x44445555
...
blt t0,a2,sorteia #t0 √© o contador de numeros sorteados: if(t0<N) 
...
mv t0,zero
#Codigo que faz a serapaÁ„o de x e y no vetor principal
VECTOR:
...
#CÛdigo que faz o calculo do modulo
fmul.s f1,f1,f1
fmul.s f2,f2,f2
fadd.s f3,f2,f1
fsqrt.s f4,f3 # Colocamos o valor do mÛdulo em f4
...
##Codigo que faz a ordenaÁ„o do vetor de mÛdulos

MAIN:	mv a0,s0 #mudei aqui para ir para nosso inicio do VETOR DE MODULOS ( cuidado para n„o enviar a word mas o endereÁo)
	li a1,N
	jal SHOW
...
loop1: 	beq t2,t1,fim1
	li a7,2 #alterei aqui para li a7, 2 que È imprimir FLOAT
