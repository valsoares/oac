...
mv t6,zero
	...
	la s1,tela		# endereço dos dados da tela na memoria
	addi s1,s1,8		# primeiro pixels depois das informações de nlin ncol
LOOP1: 	beq t1,t2,SELECAO		# Se for o último endereço então sai do loop
...
SELECAO:#colocar aqui uma função que faz ele ler do teclado o numero 1 ou 2 e vai para a a função facil ou dificil
	addi a0,a0,2000
...
sw s3,0(a6)
#sb s2,0(a6) ajeita o bug inicial de acabar se comendo no inicio do game, por nao saber onde esta a cabeça
sw s3,4(a6)
addi a6,a6,320
addi t6,t6,320
blt t6,t1,cobra
...
li t6,0 # ta com um bug aqui que as vezes ele zera t6 e volta pro Loop de cima!
jal score
jal quadrado8
jal anda
...
morreu:
	...
	li t1,0xFF000000	# endereco inicial da Memoria VGA - Frame 0
	li t2,0xFF012C00	# endereco final 
	la s1,wasted		# endereço dos dados da tela na memoria
	addi s1,s1,8		# primeiro pixels depois das informações de nlin ncol
LOOP2: 	beq t1,t2,espera		# Se for o último endereço então sai do loop
	lw t3,0(s1)		# le um conjunto de 4 pixels : word
	sw t3,0(t1)		# escreve a word na memória VGA
	addi t1,t1,4		# soma 4 ao endereço
	addi s1,s1,4
